...
export function PurchaseCard({ purchase }: { purchase: PurchaseView }) {
  ...
  return <article className="passport purchase-receipt"><div className="passport-notch" aria-hidden="true" /><p className="passport-kicker"><span className="verified-mark">✓</span> Verified with Nimiq</p><p className="passport-merchant">{purchase.merchantName}</p><h1>{purchase.itemSummary}</h1>{purchase.itemDescription && <p className="item-description">{purchase.itemDescription}</p>}<div className="passport-price"><strong>{formatNim(purchase.expectedAmountLuna)}</strong>{purchaseDate && <span>{purchaseDate}</span>}</div>
    ...
    <details className="transaction-details"><summary>Transaction details</summary><p>Verified direct NIM payment</p><p className="reference">Reference: {purchase.chainReference}</p></details><section className="support-panel" aria-labelledby="support-heading"><p className="section-label" id="support-heading">Need help?</p><p>Send a message to {purchase.merchantName}. Your payment stays direct and cannot be reversed here.</p><label htmlFor="support-message">Your message</label><textarea id="support-message" value={message} maxLength={1000} onChange={(event) => setMessage(event.target.value)} placeholder="How can the merchant help?" /><button disabled={!message.trim() || sending} onClick={() => void support()}>{sending ? 'Sending…' : 'Get support'}</button>{notice && <p className="support-notice" role="status">{notice}</p>}</section>
  </article>
}
