...
export function PurchaseCard({ purchase }: { purchase: PurchaseView }) {
  ...
  return <article className="purchase-card purchase-receipt"><p className="eyebrow">Verified with Nimiq</p><h1>{purchase.itemSummary}</h1>{purchase.itemDescription && <p className="item-description">{purchase.itemDescription}</p>}<p className="merchant-name">{purchase.merchantName}</p><p className="amount">{formatNim(purchase.expectedAmountLuna)}</p>
