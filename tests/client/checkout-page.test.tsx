...
describe('Nimiq checkout', () => {
  ...
  it('keeps a temporary verification failure recoverable', async () => { vi.useFakeTimers(); mocks.get.mockResolvedValue(purchase('PAYMENT_SUBMITTED')); mocks.verify.mockRejectedValue(new Error('RPC unavailable')); render(<CheckoutPage purchaseId="purchase-1" />); await act(async () => { await Promise.resolve(); await vi.advanceTimersByTimeAsync(3_000) }); expect(screen.getByRole('alert').textContent).toContain('RPC unavailable') })
