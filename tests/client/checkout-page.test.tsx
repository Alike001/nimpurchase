...
beforeEach(() => { window.localStorage.clear(); mocks.verify.mockResolvedValue({ status: 'PAYMENT_SUBMITTED' }) })
...
describe('Nimiq checkout', () => {
  ...
  it('remembers an active Passport on this device', async () => { mocks.get.mockResolvedValue(purchase('ACTIVE')); render(<CheckoutPage purchaseId="purchase-1" />); await screen.findByText('Purchase verified'); expect(window.localStorage.getItem('nimpurchase:recent-passports')).toContain('purchase-1') })
